...
const MovieDetails = ({ getMovie, isLoading, match: {params}, movie, isError }) => {
  ...
  const countSeasons = () => {
    let seasons = [];
    for(let i = 1; i <= Number(movie.totalSeasons); i++) {
      seasons.push(<NavLink to={`/movie/${movie.imdbID}/season/${i}`}><span key={i}>{i}</span></NavLink>);
    }
    return seasons;
  }
          ...
          <div className="film-card">       
            ...
            <div className="film-card__info">
              ...
              <div className="film-card__about">
                ...
                {movie.totalSeasons && <span className="film-card__about-item">{`${movie.totalSeasons} seasons`}</span>}
              </div>
